...
import com.open.chitchat.model.Data.Event.EventMessage;
	...
	public void handleEvent(Message message) {
		...
		if (message.sendType.equals("event")) {
			...
			if ("message".equals(contentType)) {
				...
				if (isNotify) {
					// if
					// (NotificationUtils.isLeave(viewManage.mainView.context))
					// {
					// NotificationUtils.showMessageNotification(viewManage.mainView.context,
					// messageGson);
					// } else {
...
//		viewManage.postNotifyView("MessagesSubView");
	...
	public void updateLocalMessage(Message message) {
		...
		if ("point".equals(sendType)) {
			...
			ArrayList<Message> messageList = data.messages.messageMap.get(key);
			if (messageList == null) {
				messageList = new ArrayList<Message>();
				data.messages.messageMap.put(key, messageList);
			}
			...
			ArrayList<Message> groupList = data.messages.messageMap.get(key);
			if (groupList == null) {
				groupList = new ArrayList<Message>();
				data.messages.messageMap.put(key, groupList);
			}
